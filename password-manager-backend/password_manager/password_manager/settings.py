...
def csv(name, default=""):
    return [
        part.strip() for part in env(name, default=default).split(",") if part.strip()
    ]
...
CSRF_TRUSTED_ORIGINS = csv(
    "CSRF_TRUSTED_ORIGINS", ",".join(CORS_ALLOWED_ORIGINS) if DEBUG else ""
)
...
for origin in CORS_ALLOWED_ORIGINS + CSRF_TRUSTED_ORIGINS:
    ...
    if (
        ...
        or parsed.fragment
        or parsed.username
        or parsed.scheme not in (("http", "https") if DEBUG else ("https",))
    ):
        raise ImproperlyConfigured(
            "CORS/CSRF origins must be explicit origins (HTTPS in production), without paths."
        )
...
REST_FRAMEWORK = {
    "DEFAULT_AUTHENTICATION_CLASSES": [
        "users.authentication.CookieSessionAuthentication"
    ],
...
if SESSION_COOKIE_SAMESITE not in ("Lax", "Strict", "None") or (
    DEBUG and SESSION_COOKIE_SAMESITE == "None"
):
    raise ImproperlyConfigured(
        "COOKIE_SAMESITE must be Lax/Strict, or None with production HTTPS."
    )
