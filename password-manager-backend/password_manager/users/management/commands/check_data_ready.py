...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        if Image.objects.exists():
            raise CommandError(
                "Legacy photos remain. Run the documented offline conversion before serving traffic."
            )
        try:
            ...
            raise CommandError(
                "Encrypted data validation failed. Restore the correct keys; no data changed."
            ) from None
