...
def decrypt(value, key_name='VAULT_ENCRYPTION_KEY'):
    if not isinstance(value, Ciphertext) or not value.startswith(PREFIX):
        raise InvalidToken
    return Fernet(getattr(settings, key_name).encode()).decrypt(value[len(PREFIX):].encode()).decode()
...
class EncryptedTextField(models.TextField):
    ...
    def get_prep_value(self, value):
        ...
        if isinstance(value, Ciphertext):
            ...
            return str(value)
        token = Fernet(getattr(settings, self.key_name).encode()).encrypt(str(value).encode())
