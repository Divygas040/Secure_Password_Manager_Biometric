...
def extract_encoding(upload):
    if upload is None or upload.size > settings.MAX_FACE_IMAGE_BYTES or upload.size == 0:
    ...
    try:
        ...
        with warnings.catch_warnings():
            ...
            with Image.open(BytesIO(data)) as image:
                if image.format not in ('JPEG', 'PNG') or image.width * image.height > settings.MAX_FACE_IMAGE_PIXELS:
                    raise ValueError
                image.load()
                image.thumbnail((800, 800))
                pixels = np.array(image.convert('RGB'))
    except (ValueError, OSError, UnidentifiedImageError, Image.DecompressionBombError, Image.DecompressionBombWarning):
        raise ValidationError('Provide a valid JPEG or PNG within the image limits.') from None
    try:
...
def matches(stored, submitted):
    saved = np.asarray(json.loads(decrypt(stored, 'BIOMETRIC_ENCRYPTION_KEY')), dtype=float)
    candidate = np.asarray(submitted, dtype=float)
    if saved.shape != (128,) or candidate.shape != (128,) or not np.isfinite(saved).all() or not np.isfinite(candidate).all():
        return False
